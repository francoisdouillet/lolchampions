...
function Difficulty({ formData, setFormData, setPage }) {
  ...
  /*function onSubmit() {
    //setFormData({...formData, commentary: commentary})
    if(formData.difficulty === "") {
      setError(true)
    }
    localStorage.setItem(
      `${formData.myChamp}VS${formData.hisChamp}`,
      JSON.stringify({ ...formData, commentary: commentary })
    );
  }*/
  async function onSubmit() {
    ...
    try {
      const response = await axios.post("http://localhost:3000/api/add/matchup", {...formData, commentary: commentary})
  ...
  return (
    <>
      ...
      <div className="formadd__checkbox">
        <div>
          <h3>Game gagné ?</h3>
          <Checkbox checked={formData.win == true} onChange={(e) => {
            setFormData({...formData, win: e.target.checked})
          }}/>
        </div>
        <div>
          <h3>Match-up gagné ?</h3>
          <Checkbox checked={formData.winMatchup == true} onChange={(e) => {
